...
<#macro main ele>
	...
	<#if util.isFolder(ele)>
		...
		<#list tableResources as tableResource>
            ...
            <#if !isFilePathSet>
		        ${fileUtil.setFile(path + "/" + tableInfo.getSchema() + "_oracle_patch.sql")}<#t>
                <#local isFilePathSet = true/>
            </#if>
			<#local tableName = tableInfo.getName()>
			-- begin 表${tableName}增量脚本<#lt>
			<@oracle.genTablePatchSql4Oracle tableInfo = tableInfo/>
	...
	<#else>
		...
		${fileUtil.setFile(path + "/" + tableInfo.getName() + "_oracle_patch.sql")}
		<@oracle.genTablePatchSql4Oracle tableInfo = tableInfo/>
	</#if>
</#macro>