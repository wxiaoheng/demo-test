<#--
HEPType:初始数据
HEPName:生成DB2数据库表的初始数据增量SQL
-->
<#import "db_gensql_macro.ftl" as db2>
<#assign path = util.openDirectory("")>
<#if stringUtil.isBlank(path)>
	${util.dialog('生成DB2数据库表的初始数据增量SQL！')}<#t>
<#elseif stringUtil.equals(path, "PREVIEW")>
	<#-- 预览模式，直接生成代码，不生成文件 -->
	<@db2.genTableInsertPatchSql4DB2 info = model tableName = tableName/>
<#else>
	<#-- 调用宏指令 -->
	<@main element/>
</#if>
...
<#macro main ele>
	...
	<#if util.isFolder(ele)>
		...
		<#list dataResources as dataResource>
			...
			<#if !isFilePathSet>
		        ${fileUtil.setFile(path + "/" + stringUtil.createJoiner("_").add(ele.getName()).add(db2.getDbSchema(tableName)).toString() + "_db2_insert_patch.sql")}<#t>
                <#local isFilePathSet = true/>
            </#if>
			-- begin 表${tableName}初始数据增量脚本<#lt>
			<@db2.genTableInsertPatchSql4DB2 info = info tableName = tableName/>
	...
	<#else>
		...
		${fileUtil.setFile(path + "/" + tableName + "_db2_insert_patch.sql")}
		-- begin 表${tableName}初始数据增量脚本<#lt>
		<@db2.genTableInsertPatchSql4DB2 info = info tableName = tableName/>
		-- end 表${tableName}初始数据增量脚本<#lt>
	</#if>
