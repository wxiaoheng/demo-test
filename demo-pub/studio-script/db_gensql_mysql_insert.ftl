<#--
HEPType:初始数据
HEPName:生成MySql数据库表的初始数据
-->
<#import "db_gensql_macro.ftl" as mysql>
<#assign path = util.openDirectory("")>
<#if stringUtil.isBlank(path)>
	${util.dialog('生成MySql数据库表的初始数据取消！')}<#t>
<#elseif stringUtil.equals(path, "PREVIEW")>
	<#-- 预览模式，直接生成代码，不生成文件 -->
	<@mysql.genTableInsertSql4MySQL resource=element info = model tableName = tableName/>
<#else>
	<#-- 调用宏指令 -->
	<@main element/>
</#if>
...
<#macro main ele>
	...
	<#if util.isFolder(ele)>
		...
		<#list dataResources as dataResource>
			...
			<#if !isFilePathSet>
		        ${fileUtil.setFile(path + "/" + stringUtil.createJoiner("_").add(ele.getName()).add(mysql.getDbSchema(tableName)).toString() + "_mysql_insert.sql")}<#t>
                <#local isFilePathSet = true/>
            </#if>
			-- begin 表${tableName}初始数据脚本<#lt>
			<@mysql.genTableInsertSql4MySQL resource = dataResource info = info tableName = tableName/>
	...
	<#else>
		...
		${fileUtil.setFile(path + "/" + tableName + "_mysql_insert.sql")}
		-- begin 表${tableName}初始数据脚本<#lt>
		<@mysql.genTableInsertSql4MySQL resource=ele info = info tableName = tableName/>
		-- end 表${tableName}初始数据脚本<#lt>
	</#if>
</#macro>