<#--
HEPType:初始数据
HEPName:生成Oracle数据库表的初始数据增量SQL
-->
<#import "db_gensql_macro.ftl" as oracle>
<#assign path = util.openDirectory("")>
<#if stringUtil.isBlank(path)>
	${util.dialog('生成Oracle数据库表的初始数据增量SQL取消！')}<#t>
<#elseif stringUtil.equals(path, "PREVIEW")>
	<#-- 预览模式，直接生成代码，不生成文件 -->
	<@oracle.genTableInsertPatchSql4Oracle resource=element info = model tableName = tableName/>
<#else>
	<#-- 调用宏指令 -->
	<@main element/>
</#if>
...
<#macro main ele>
	...
	<#if util.isFolder(ele)>
		...
		<#list dataResources as dataResource>
			...
			<#if !isFilePathSet>
		        ${fileUtil.setFile(path + "/" + stringUtil.createJoiner("_").add(ele.getName()).add(oracle.getDbSchema(tableName)).toString() + "_oracle_insert_patch.sql")}<#t>
                <#local isFilePathSet = true/>
            </#if>
			-- begin 表${tableName}初始数据增量脚本<#lt>
			<@oracle.genTableInsertPatchSql4Oracle resource=dataResource info = info tableName = tableName/>
	...
	<#else>
		...
		${fileUtil.setFile(path + "/" + tableName + "_oracle_insert_patch.sql")}
		-- begin 表${tableName}初始数据增量脚本<#lt>
		<@oracle.genTableInsertPatchSql4Oracle resource=ele info = info tableName = tableName/>
		-- end 表${tableName}初始数据增量脚本<#lt>
	</#if>
</#macro>