...
<#if needSetTotalAndCurrentPage>
    ...
    <#if (customConditionMap?size) != 0>
        <#assign sqlCountTotal = sqlCountTotal+"\" + sql + \""+")">
    <#else>
        <#assign sqlCountTotal = sqlCountTotal+sql+")">
    </#if>
...
<#if stringUtil.isNotBlank(pageNo)>
    ...
    <#assign sqlSelect = "select * from (select Res.*, rownum as rn from (">
    <#assign sqlSelect1 = "select * from (">
    <#if (customConditionMap?size) != 0>
        <#assign sqlSelect = sqlSelect+"\" + sql + \""+groupOrderByStatement+") Res where rownum <= :pageNo * :pageSize ) where rn > (:pageNo - 1) * :pageSize">
        <#assign sqlSelect1 = sqlSelect1+"\" + sql + \""+groupOrderByStatement+") where rownum <= :rownum">
    <#else>
        <#assign sqlSelect = sqlSelect+sql+groupOrderByStatement+") Res where rownum <= :pageNo * :pageSize ) where rn > (:pageNo - 1) * :pageSize">
        <#assign sqlSelect1 = sqlSelect1+sql+groupOrderByStatement+") where rownum <= :rownum">
    </#if>
    ...
    if (${realNo} > 0) {
        ...
        <#else>  
        sql = "${sqlUtil.format(sqlSelect)}";
        mapSqlParameterSource.addValue("pageNo", ${realNo}).addValue("pageSize", ${realSize});
        </#if>
    } else {
        <#-- 如果不存在动态条件 ，则使用jdbcTemplate-->
        <#if (customConditionMap?size) == 0>
...
<#else>
    <#assign sqlBlank = "select * from (">
    <#if (customConditionMap?size) != 0>
        <#assign sqlBlank = sqlBlank+"\" + sql + \""+groupOrderByStatement+") where rownum <= :rownum">
    <#else>
        <#assign sqlBlank = sqlBlank+sql+groupOrderByStatement+") where rownum <= :rownum">
    </#if>
    <#-- 如果不存在动态条件 ，则使用jdbcTemplate-->    
