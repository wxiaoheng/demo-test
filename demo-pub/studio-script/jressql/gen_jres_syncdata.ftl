...
${builder.addImport("org.springframework.jdbc.core.namedparam.MapSqlParameterSource")}<#t>
...
<#assign sqlInsert = "insert into "+tableName+"("+columnStr+") values("+columnValueStr+")">
    ...
    Map<String, Object> indexMap = message.getIndexes();
            ...
            if (data == null) {
                dataNullSyncData(indexMap);
       		} else {
                @SuppressWarnings("unchecked")
                Map<String, Object> dataMap = (Map<String, Object>)data;
                if (exists${capTableNameFirst}(${inputGetParam})) {
                    updateSyncData(dataMap,indexMap);
                } else {
                    insertSyncData(dataMap, indexMap);
                }
            }
}
   
private void dataNullSyncData(Map<String, Object> indexMap) {
...
private void updateSyncData(Map<String, Object> dataMap, Map<String, Object> indexMap) {
...
<#list columns as column>
    <#assign columnName = column.getName()/> 
    updateSyncField(isAppended, ${stringUtil.toUpperCase(columnName)}, builder, mapSqlParameterSource, dataMap);
...
private void updateSyncField(boolean isAppended, String field, StringBuilder builder, MapSqlParameterSource mapSqlParameterSource, Map<String, Object> dataMap){
	if (dataMap.containsKey(field)) {
	    ...
	    isAppended = true;
	    mapSqlParameterSource.addValue(field, dataMap.get(field));
    }
}

private void insertSyncData(Map<String, Object> dataMap, Map<String, Object> indexMap) {
