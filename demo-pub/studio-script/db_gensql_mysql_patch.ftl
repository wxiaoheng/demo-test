...
<#macro main ele>
	...
	<#if util.isFolder(ele)>
		...
		<#list tableResources as tableResource>
            ...
            <#if !isFilePathSet>
		        ${fileUtil.setFile(path + "/" + tableInfo.getSchema() + "_mysql_patch.sql")}<#t>
                <#local isFilePathSet = true/>
            </#if>
			<#local tableName = tableInfo.getName()>
			-- begin 表${tableName}增量脚本<#lt>
			<@mysql.genTablePatchSql4MySQL tableInfo = tableInfo/>
	...
	<#else>
		...
		${fileUtil.setFile(path + "/" + tableInfo.getName() + "_mysql_patch.sql")}<#t>
		<@mysql.genTablePatchSql4MySQL tableInfo = tableInfo/>
	</#if>
</#macro>